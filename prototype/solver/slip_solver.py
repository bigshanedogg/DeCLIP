...
class ClsSolver(BaseSolver):
    ...
    def setup_env(self):
        ...
        if hasattr(self.config.saver, 'pretrain'):
            ...
            if self.config.saver.pretrain.get("path", None):
                ...
                self.state['last_iter'] = self.state.get('last_iter', 0)
            else:
                self.state = {}
    ...
    def pre_train(self):
        ...
        self.meters.simclr_losses = AverageMeter(self.config.saver.print_freq)
        self.meters.clip_dense_losses = AverageMeter(self.config.saver.print_freq)
        self.meters.mae_losses = AverageMeter(self.config.saver.print_freq)
        ...
        self.simclr_criterion = NT_Xent_gather(self.config.data.batch_size)
        self.nt_xent_criterion = NT_Xent(self.config.data.batch_size)
    ...
    def train(self):
        ...
        for i in range(len(self.train_data['loader'])):
            ...
            if 'CLSA' not in self.config.data.train.transforms.type:
                ...
                if 'slip' in self.config.model.type:
                    logits_per_image, logits_per_text = output_dict['logits']
                    text_features, image_features = output_dict['features']
                    ...
                    if self.config.data.train.get('image_text_two_view', False):
                ...
                if 'dense_logits' in output_dict.keys():
                    ...
                    clip_dense_loss = clip_dense_loss / self.dist.world_size
                else:
                    clip_dense_loss = torch.zeros_like(clip_loss)

                if 'masked_autoencoder_loss' in output_dict.keys():
                    mae_loss = output_dict['masked_autoencoder_loss'] / self.dist.world_size
                else:
                ...
                if 'slip' not in self.config.model.type:
                    ...
                    # print(clip_loss, simclr_loss, '<< losses', nt_xent_loss, flush=True)
                else:
                    nt_xent_loss = self.nt_xent_criterion(image_features, text_features) / self.dist.world_size

            else:
                raise NotImplementedError('Solver Codes Not Implemented! please check it')

            if not self.config.loss_weight.get('type', None):
                loss = clip_loss * self.config.loss_weight.clip_loss
                if self.config.loss_weight.get('simclr_loss', 0):
                    loss = loss + simclr_loss * self.config.loss_weight.simclr_loss
                if self.config.loss_weight.get('masking_language', 0):
                    loss = loss + text_mlm_loss * self.config.loss_weight.masking_language
                    # print('New Loss, text_mlm_loss', text_mlm_loss, flush=True)
                if self.config.loss_weight.get('clip_dense_loss', 0):
                    loss = loss + clip_dense_loss * self.config.loss_weight.clip_dense_loss
                    # print('New Loss, clip_dense_loss', clip_dense_loss, flush=True)
                if self.config.loss_weight.get('mae_loss', 0):
                    loss = loss + mae_loss * self.config.loss_weight.mae_loss
                if self.config.loss_weight.get('nn_text', 0):
                    loss = loss + clip_nn_text_loss * self.config.loss_weight.nn_text
                    # print(clip_nn_text_loss, '<< nn text loss', flush=True)
            elif self.config.loss_weight.get('type', None) == 'linear':
            ...
            reduced_simclr_loss = simclr_loss.clone()
            reduced_clip_dense_losses = clip_dense_loss.clone()
            reduced_mae_losses = mae_loss.clone()
            ...
            self.meters.simclr_losses.reduce_update(reduced_simclr_loss)
            self.meters.clip_dense_losses.reduce_update(reduced_clip_dense_losses)
            self.meters.mae_losses.reduce_update(reduced_mae_losses)
            ...
            if curr_step % self.config.saver.print_freq == 0 and self.dist.rank == 0:
                ...
                self.tb_logger.add_scalar(
                    'simclr_loss_train', self.meters.simclr_losses.avg, curr_step)
                self.tb_logger.add_scalar(
                    'nt_xent_loss_train', self.meters.nt_xent_losses.avg, curr_step)
                self.tb_logger.add_scalar(
                    'text_mlm_loss', self.meters.text_mlm_losses.avg, curr_step)
                self.tb_logger.add_scalar(
                    'clip_dense_loss_train', self.meters.clip_dense_losses.avg, curr_step)
                self.tb_logger.add_scalar(
                    'mae_loss_train', self.meters.mae_losses.avg, curr_step)
                self.tb_logger.add_scalar(
